...
def passworddb_helper(connection, accusername):
    """Require."""
...
def is_login(func):
    ...
    def guard(*args, **kwargs):
        ...
        if auth:
            ...
            passworddb = passworddb_helper(connection, accusername)

            if passworddb:
                temppassword110 = passworddb[0]['password']
                algorithm = 'sha512'
                _, salt, _ = temppassword110.split('$')
                hash_obj = hashlib.new(algorithm)
                password_salted12345 = salt + accpassword
                hash_obj.update(password_salted12345.encode('utf-8'))
                password_hash = hash_obj.hexdigest()
                password_db_string = "$".join(
                    [algorithm, salt, password_hash]
                )

                if password_db_string != temppassword110:
