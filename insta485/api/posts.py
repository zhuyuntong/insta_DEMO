...
def totalpost_helper(connection, accusername):
    """Require."""
...
def getpostdb_helper(
    connection, accusername, postid_lte, size, offset
):
    """Require."""
    cur2 = connection.execute(
        "SELECT p.postid "
...
def get_post():
    """Return post."""
    # TBD later
    auth123 = request.authorization
    accusername = ''
    if auth123:
        accusername = auth123['username']
    else:
    ...
    totalpost = totalpost_helper(connection, accusername)
...
def get_postid(postid_url_slug):
    """Require."""
    # TBD later
    auth321 = request.authorization
    accusername = ''
    if auth321:
        accusername = auth321['username']
    else:
        accusername = session['username']
    # Username
    connection = insta485.model.get_db()
