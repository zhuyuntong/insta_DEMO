...
def accounts_create():
    ...
    if not data:
        ...
        connection.execute(
            ...
            "VALUES (?, ?, ?, ?, ?)",
            (accusername, accfullname, accemail,
...
def accounts_delete():
    ...
    for post in postfiles:
        os.remove(Path(insta485.app.config["UPLOAD_FOLDER"]/post['filename']))
    ...
    os.remove(
        Path(insta485.app.config["UPLOAD_FOLDER"]/userfiles[0]['filename'])
    )
...
def edit():
    ...
    for user in userdb:
        user['filename'] = url_for('get_file', filename=user['filename'])
    context = {
        'logname': accusername,
...
def accounts_edit():
    ...
    if fileobj.filename != '' and fileobj:
        ...
        os.remove(Path(insta485.app.config["UPLOAD_FOLDER"] / origfilename))
...
def accounts_update():
    ...
    connection.execute(
        ...
        "SET password=? "
        "WHERE username=?",
        (password_db_string, accusername, )
    )
