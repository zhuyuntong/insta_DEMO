...
def post_likes():
    """Return post."""
    # TBD later
    auth555 = request.authorization
    accusername = ''
    if auth555:
        accusername = auth555['username']
    else:
    ...
    likeid = cur.fetchall()
    templikeid = likeid[0]['last_insert_rowid()']
    context['likeid'] = templikeid
    context['url'] = '/api/v1/likes/' + str(templikeid) + '/'
