...
def get_post():
  ...
  cur2 = connection.execute(
        ...
        "LIMIT ? OFFSET ?",
        (accusername, postid_lte, accusername, postid_lte, size, offset, )
  )
  ...
  if totalpost[0]['COUNT(postid)'] > (size * (page + 1)) or totalpost[0]['COUNT(postid)'] % (tempsize * (page + 1)) == 0:
...
def get_postid(postid_url_slug):
    ...
    likesowner = likesowner_helper(connection, postid_url_slug, accusername)
    ...
    if likes['lognameLikesThis'] == True:
      likes['url'] = '/api/v1/likes/' + str(likesowner[0]['likeid']) + '/'
    ...
    if postsdb:
      ...
      context['ownerImgUrl'] = '/uploads/' + postsdb[0]['ownerImgUrl']
      context['ownerShowUrl'] = '/users/' + postsdb[0]['owner'] + '/'
      context['postShowUrl'] = '/posts/' + str(postsdb[0]['postid']) + '/'
      context['postid'] = postsdb[0]['postid']
      context['url'] = '/api/v1/posts/' + str(postsdb[0]['postid']) + '/'
    else:
